    ...
    <div class="card_content">
        <div class="card_body">
            <div class="card_tag">
                <a href="" class="card_category" style="display:inline-block; margin-bottom:10px;">{{ $category->nama }}</a>
                @auth
                    <form action="{{ route($product->wishlists()->where('user_id', auth()->id())->exists() ? 'wishlist.remove' : 'wishlist.add', $product->id) }}" method="{{ $product->wishlists()->where('user_id', auth()->id())->exists() ? 'DELETE' : 'POST' }}" style="display:inline;">
                        @csrf
                        @if($product->wishlists()->where('user_id', auth()->id())->exists())
                            @method('DELETE')
                        @endif
                        <button type="submit" class="card_like" style="color: {{ $product->wishlists()->where('user_id', auth()->id())->exists() ? 'red' : 'grey' }};">
